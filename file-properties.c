...
int compute_file_md5(files_list_entry_t *entry) {
    ...
    // Création d'un contexte de hachage MD5
    EVP_MD_CTX *md_ctx = EVP_MD_CTX_new();

    // Vérification si la création du contexte MD5 a réussi
    if (md_ctx == NULL) {
        perror("Error creating MD context");
        fclose(file);
        return -1;
    }

    // Récupération de l'algorithme MD5
    const EVP_MD *md = EVP_md5();

    // Vérification si la récupération de l'algorithme MD5 a réussi
    if (md == NULL) {
    ...
    // Initialisation du contexte de hachage MD5
    if (1 != EVP_DigestInit_ex(md_ctx, md, NULL)) {
        perror("Error initializing MD context");
        EVP_MD_CTX_free(md_ctx);
        fclose(file);
        return -1;
    }

    // Lecture du fichier par blocs et mise à jour du contexte de hachage MD5
    unsigned char buffer[4096];
    size_t bytesRead;

    while ((bytesRead = fread(buffer, 1, sizeof(buffer), file)) > 0) {
        if (1 != EVP_DigestUpdate(md_ctx, buffer, bytesRead)) {
            perror("Error updating MD context");
            EVP_MD_CTX_free(md_ctx);
            fclose(file);
            return -1;
        }
    ...
    // Finalisation du contexte de hachage MD5
    if (1 != EVP_DigestFinal_ex(md_ctx, entry->md5sum, NULL)) {
        perror("Error finalizing MD context");
        EVP_MD_CTX_free(md_ctx);
        fclose(file);
        return -1;
    }

    // Libération des ressources utilisées par le contexte MD5
    EVP_MD_CTX_free(md_ctx);
